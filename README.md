...
```bash
pi install git:github.com/ac5tin/pi-subagents
```

Same shape as other git Pi packages (`git:github.com/obra/superpowers`, `git:github.com/DietrichGebert/ponytail`): `pi update --extensions` pulls the latest commit. `pi install npm:@ac5tin/pi-subagents` still works if you want a versioned npm release.
...
Frontmatter is authoritative. If an agent file sets `model`, `thinking`, `max_turns`, `inherit_context`, `run_in_background`, `isolated`, or `isolation`, those values are locked for that agent. `Agent` tool parameters only fill fields the agent config leaves unspecified. If both the call and the agent file omit `model` or `thinking`, that field is copied from the live parent session — not from pi `settings.json` defaults. Resume still replays the stored session.
